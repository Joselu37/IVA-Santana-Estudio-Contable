    ...
    for (let i = headerLineIdx + 1; i < lines.length; i++) {
      ...
      if (neto === 0) {
        ...
        cols.forEach((colVal, cIdx) => {
          if (columnasExcluidas.includes(cIdx)) return;
          const soloDigitos = String(colVal).replace(/\D/g, '');
          // Si la columna, sin separadores, es una cadena de 11 dígitos (con o sin
          // guiones tipo CUIT), NUNCA la tomamos como importe: es un identificador,
          // no un monto, aunque no haya sido reconocida como la columna de CUIT.
          if (soloDigitos.length === 11) return;
          const valNum = parseArgNumber(colVal);
          if (valNum > 100 && neto === 0) {
            neto = valNum;
          }
        });
      }
